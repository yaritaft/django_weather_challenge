...
def accuweather():
    ...
    try:
        ...
        lat, lon = (
            float(request.args.get(latitude_param)),
            float(request.args.get(longitude_param)),
        )
    except:
        return make_response(
            'Must provide the "{:s}" and "{:s}" query params as float values\n'.format(
...
def noaa():
    ...
    try:
        ...
        return make_response(
            'Must provide the "{:s}" query parameter in the form "30.2335595,-97.7797051"\n'.format(
                param_name
            ),
            400,
        )
